'use strict';
//from here: https://github.com/etiennepinchon/aframe-rounded

AFRAME.registerComponent('circles-rounded-rectangle', {
    schema: {
    ...
    init: function () {
      this.rounded = new THREE.Mesh( this.createShape(), new THREE.MeshPhongMaterial( { color: new THREE.Color(this.data.color), side: THREE.DoubleSide } ) );
      this.updateOpacity();
    ...
    update: function () {
      if (this.data.enabled) {
        if (this.rounded) {
          ...
          this.rounded.geometry = this.createShape();
