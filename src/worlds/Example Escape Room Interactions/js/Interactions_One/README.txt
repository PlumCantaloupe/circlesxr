...
Create 3 AFrame entities that have the ‘placementSpots’ component and make sure that the id of the entities each match one of the requiredSpot variables in the already constructed objectInfo class variables found in the HoldObject.js and has circles-interactive-object="type:highlight".
...
Create 1 AFrame entity with the id “ScreenTextPlacementSpot” and set the default text to “missing objects” (This can be changed). This text is made to inform users with a visual cue that they completed the puzzle.
...
Note: A quicker method to set up the Interaction is to go into the index file found in the Soical_Interactions world folder and copy and paste, lines 41 to 104. Doing this would only require changes to the AFrame entity (model, texture colour, ect).
...
Puzzle1SolvedUpdateScreens() is the function that updates the player with a visual cue, within the function itself you can change text and color that it changes to.
